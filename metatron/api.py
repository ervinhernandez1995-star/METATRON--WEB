...
"""
╔══════════════════════════════════════════════════════════════╗
║        METATRON v2.3 — api.py  (WSL2 / ngrok EDITION)        ║
...
║  • /list-networks: decorador @app.get restaurado             ║
║  • /analyze: agrega full_response+exploits+raw_scan_preview  ║
║    para compatibilidad total con index.html renderScanResults ║
╚══════════════════════════════════════════════════════════════╝
...
def get_connection():
    conn = mysql.connector.connect(
        host="172.31.165.91",      # IP de tu WSL2
        user="metatron",           # usuario creado
        password="1234",
        database="metatron_db"     # tu base de datos
    )
    return conn
    ...
    def scan_wifi_powershell():
        ...
        def flush_bssid():
            ...
            if current_bssid_block and current_ssid:
                # Construir el campo security de forma limpia
                _auth    = current_auth    if current_auth    != "N/A" else ""
                _encrypt = current_encrypt if current_encrypt != "N/A" else ""
                if _auth and _encrypt:
                    _security = f"{_auth} / {_encrypt}"
                elif _auth:
                    _security = _auth
                elif _encrypt:
                    _security = _encrypt
...
# ── 2. CORSMiddleware ─────────────────────────────────────────
# Configuración CORS para producción con Vercel + ngrok
# NOTA: allow_credentials=True requiere allow_origins específico (no "*")
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://metatron-74jzfj6c3-erins-projects-50c165ad.vercel.app",
        "http://localhost:3000",
        "http://localhost:3001",
    ],
...
class NetworkActionRequest(BaseModel):
    """Peticion de accion sobre una red Wi-Fi detectada."""
    action: str          # "info" | "ping" | "portscan" | "analyze" | "deauth"
    ssid:   str
    bssid:  Optional[str] = "N/A"
    target: Optional[str] = None   # override de target si se quiere
...
def root():
    return {
        ...
        "model":     MODEL,
        "endpoints": [
            "/scan", "/analyze", "/network-scan", "/network-action", "/list-networks",
            "/create-skill", "/skills", "/run-skill",
            "/history", "/session/{sl_no}", "/knowledge", "/debug-scan"
        ]
    }
...
def analyze_ip(req: AnalyzeRequest):
    ...
    return {
        ...
        "analysis":        result.get("full_response",""),   # ← script.js original usa este campo
        "full_response":   result.get("full_response",""),   # ← index.html renderScanResults usa este
        "vulnerabilities": result.get("vulnerabilities",[]),
        "exploits":        result.get("exploits",[]),
        "raw_scan_preview": raw_scan[:1000],                 # ← index.html tab RAW SCAN
    }
...
# ── /network-action  ─────────────────────────────────────────
# Endpoint dedicado para acciones sobre redes Wi-Fi desde el dashboard.
# El frontend puede llamar directamente a /scan, pero este endpoint
# centraliza la logica y devuelve un formato estandarizado.
@app.post("/network-action")
def network_action(req: NetworkActionRequest):
    """Ejecuta una accion sobre una red Wi-Fi detectada."""
    action = req.action.lower()
    ssid   = req.ssid.strip()
    bssid  = (req.bssid or "").strip()

    if action == "info":
        # Buscar en DB
        conn = get_connection(); c = conn.cursor(dictionary=True)
        c.execute(
            "SELECT * FROM wifi_networks WHERE ssid=%s OR bssid=%s ORDER BY id DESC LIMIT 1",
            (ssid, bssid)
        )
        row = _safe_row(c.fetchone()); conn.close()
        return {"action": "info", "ssid": ssid, "bssid": bssid, "record": row}

    elif action == "ping":
        out = MetatronEngine._run("ping -c 3 8.8.8.8 2>&1 || ping -n 3 8.8.8.8", timeout=15)
        return {"action": "ping", "ssid": ssid, "output": out}

    elif action == "portscan":
        target = req.target or bssid or "192.168.1.1"
        if target == "N/A": target = "192.168.1.1"
        out = MetatronEngine.run_nmap(target, "service")
        return {"action": "portscan", "target": target, "output": out}

    elif action in ("analyze", "attack"):
        target = req.target or ssid
        raw    = MetatronEngine.run_nmap(target, "service")
        result = MetatronEngine.analyse(target, raw)
        sl_no  = db_create_session(target)
        db_save_summary(sl_no, raw, result.get("full_response",""), result.get("risk_level","UNKNOWN"))
        return {
            "action":          action,
            "ssid":            ssid,
            "sl_no":           sl_no,
        ...
        return {
            ...
            "ssid":   ssid,
            "bssid":  bssid,
            "note":   "SIMULATION ONLY — no real deauth executed",
            "cmd":    f"aireplay-ng --deauth 10 -a {bssid} wlan0mon",
        }

    else:
        raise HTTPException(400, f"Accion desconocida: {action}. Opciones: info, ping, portscan, analyze, attack, deauth")
...
if __name__ == "__main__":
    ...
    ██║ ╚═╝ ██║███████╗   ██║   ██║  ██║   ██║   ██║  ██║╚██████╔╝██║ ╚████║
    ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝
\033[0m    \033[90mv2.3 — FastAPI Bridge | Groq llama3-70b | WSL2 (Integración full-stack completa)\033[0m
    """)
