...
class MetatronEngine:
    ...
    def scan_wifi_powershell():
        ...
        # ── Regex patterns (EN + ES) ──────────────────────────
        RE_SSID     = re.compile(r"^SSID\s+\d+\s*:\s*(.*)$", re.IGNORECASE)
        RE_AUTH     = re.compile(r"^(Authentication|Autenticaci[oó]n)\s*:\s*(.*)$", re.IGNORECASE)
        RE_ENCRYPT  = re.compile(r"^(Encryption|Cifrado)\s*:\s*(.*)$", re.IGNORECASE)
        RE_BSSID    = re.compile(r"^BSSID\s+\d+\s*:\s*(.*)$", re.IGNORECASE)
        RE_SIGNAL   = re.compile(r"^(Signal|Se[ñn]al)\s*:\s*(.*)$", re.IGNORECASE)
        RE_CHANNEL  = re.compile(r"^(Channel|Canal)\s*:\s*(.*)$", re.IGNORECASE)
        RE_RADIO    = re.compile(r"^(Radio type|Tipo de radio)\s*:\s*(.*)$", re.IGNORECASE)
